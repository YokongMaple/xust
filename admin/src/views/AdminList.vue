...
export default {
  ...
  methods: {
    ...
    async search() {
      this.items = [];
      const res = await this.$http.get(
        `http://49.232.138.118:8080/yunzhi/admin/query_admins?pageNum=${this.currentPage}&isActivate=${this.isActivate}`
      );
      this.items = res.data.data.list;
      this.total = res.data.data.total;
      this.pageSize = res.data.data.pageSize;
    ...
    async handleCurrentChange(val) {
      ...
      const res = await this.$http.get(
        `http://49.232.138.118:8080/yunzhi/admin/query_admins?pageNum=${val}&isActivate=${this.isActivate}`
      );
      this.items = res.data.data.list;
    },
    // 批量激活
